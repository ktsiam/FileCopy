...
const int DEFAULT_TIMEOUT_MS = 1; // highest speed (but also bandwidth)
...
int main(int argc, char *argv[]) {
    ...
    sock.turnOnTimeouts(DEFAULT_TIMEOUT_MS);
    ...
    for (const std::string &fname : filenames) {
        ...
        do {
            ...
            *GRADING << "File: " << fname 
            ...
            sock.turnOnTimeouts(std::max(duration/20, DEFAULT_TIMEOUT_MS));
            ...
            sock.turnOnTimeouts(DEFAULT_TIMEOUT_MS);
            if (e2e_success) {
                *GRADING << "File: " << fname << " end-to-end check succeeded, attempt " 
                         << attempt << endl;
            } else {
                *GRADING << "File: " << fname << " end-to-end check failed, attempt " 
                         << attempt << endl;
            }
        } while (!e2e_success);
    }
